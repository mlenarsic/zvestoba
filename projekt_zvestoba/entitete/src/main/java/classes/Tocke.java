...
@NamedQueries(value =
        {
                @NamedQuery(name = "classes.Tocke.getAll", query = "SELECT t FROM tocke t"),
                @NamedQuery(name = "classes.Tocke.getByPonudnik",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid"),
                @NamedQuery(name = "classes.Tocke.getByUporabnik",
                        query = "SELECT t FROM tocke t where t.uporabnik = :uporabnik "),
                @NamedQuery(name = "classes.Tocke.getTopTockeByPonudnik",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid " +
                                "and t.zbrane_tocke > :tocke order by t.zbrane_tocke"),
                @NamedQuery(name = "classes.Tocke.getById",
                        query = "SELECT t FROM tocke t where t.id_kartice = :id "),
                @NamedQuery(name = "classes.Tocke.getByUporabnikAndPonudnikId",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid " +
                            "and t.uporabnik = :u")
        })
public class Tocke {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY, generator = "tocke_id_kartice_seq")
