...
@NamedQueries(value =
        {
                @NamedQuery(name = "klasi.Tocke.getAll", query = "SELECT t FROM tocke t"),
                @NamedQuery(name = "klasi.Tocke.getByPonudnik",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid"),
                @NamedQuery(name = "klasi.Tocke.getByUporabnik",
                        query = "SELECT t FROM tocke t where t.uporabnik = :uporabnik "),
                @NamedQuery(name = "klasi.Tocke.getTopTockeByPonudnik",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid " +
                                "and t.zbrane_tocke > :tocke order by t.zbrane_tocke"),
                @NamedQuery(name = "klasi.Tocke.getById",
                        query = "SELECT t FROM tocke t where t.id_kartice = :id "),
                @NamedQuery(name = "klasi.Tocke.getByUporabnikAndPonudnikId",
                        query = "SELECT t FROM tocke t where t.ponudnik_id = :pid " +
                            "and t.uporabnik = :u")
        })
public class Tocke {
